...
ll Query(int nowNode, int qleft, int qright)
{
    ...
    /**if(qleft == qright)return Tree[nowNode].sum;*/ ///stm我怎么了写的这句 欸如果把q去掉就对了吧我试试
    if(left == right)return Tree[nowNode].sum;
    int mid = (left + right)>>1;
    pushDown(nowNode);
    if(qleft <= mid)ans += Query(nowNode<<1, qleft, qright);
    if(qright /***/>/***/ mid)ans += Query(nowNode<<1|1, qleft, qright);///我他妈的老写反=.=
    pushUp(nowNode);
    return ans;
}
