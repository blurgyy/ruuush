 ...
 */
public class CakeStefanBigIntegerBinary {
	public static void main(String[] args) {
		...
		Rational min = new Rational(2);
		Rational max = new Rational(1000);

		while (max.subtract(min).compareTo(new Rational(1,1000000)) > 0) {
			Rational mid = max.add(min).divide(2);
			Rational aTmp = cutAreaRatio(vertices, mid);
			if (aTmp.compareTo(new Rational(1).subtract(aRational)) < 0) {
				max = mid;
			} else {
				min = mid;
			}
		...
		// output
		System.out.println(max.p.doubleValue()/max.q.doubleValue());
