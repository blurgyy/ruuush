...
inline void Merge(const vector<pair<int,int>>& a, const vector<pair<int,int>>& b, vector<pair<int,int>>& res) {
  int na = a.size(), nb = b.size();
  res.reserve(na+nb);
  for (int i = 0, j = 0; i < na || j < nb; ) {
    if (i == na || (j < nb && b[j].first <= a[i].first))
      res.push_back(b[j++]);
    else 
      res.push_back(a[i++]);
  }
...
void BuildSegmentTree(int pos, int from, int to) {
  ...
  if (from == to) {
    ...
    Merge(segtree[left].nodes, segtree[right].nodes, segtree[pos].nodes);
