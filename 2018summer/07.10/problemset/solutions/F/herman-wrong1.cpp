...
// only marks nodes in cycles as safe
...
void solve()
{
  ...
  while (true)
  {
    ...
    if (vertices[head].discovered)
    {
      ...
      #endif
      // if it is a back edge, mark cycle as safe
      vector<DFSBranch>::reverse_iterator i = DFSStack.rbegin(); 
      while (i != DFSStack.rend() && i->first != head) ++i;
      if (i != DFSStack.rend()) 
      {
        ++i;
        for (vector<DFSBranch>::reverse_iterator j = DFSStack.rbegin(); j != i; ++j)
        {
          if (vertices[j->first].safe) continue;
          vertices[j->first].safe = true;
          ++nrSafe;
        }
      }  
      ++DFSStack.back().second;
