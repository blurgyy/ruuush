...
numResqueed = maxFlow' . buildGraph
  where
    maxFlow' g = maxFlow g start sink

    -- buildGraph' pi = let gr = buildGraph pi in traceShow gr gr


buildGraph :: ProblemInstance -> P.Gr (Location,Time) Capacity
