...
// Variant of thomas.cpp that does not use the size of the group to constrain the maximum flow
...
{	int bot, flow = 0, i, j;
	while (bfs(source, sink)) // Find augmenting path
...
{	int runs, run, n, start, g, s, m, r, a, b, p, t, i, j, k, x, i1, i2, safety;
	...
	for (run = 0; run < runs; run++)
	{
		...
		printf("%d\n", min(g, maxflow(start, safety))); // This is slow: maxflow could be large
	}
	return 0;
}
